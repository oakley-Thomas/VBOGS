...
def test_ncore_api_is_operator_only_and_never_returns_backend_token(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert client.get("/api/ncore/catalog", headers=HEADERS).status_code == 422
        token_headers = {**HEADERS, "X-VBOGS-HF-Token": "hf-private-token"}
        catalog = client.get("/api/ncore/catalog", headers=token_headers)
        assert catalog.json()["clips"] == [{"scene_id": SCENE_ID, "status": "missing"}]
        created = client.post("/api/ncore/downloads", headers=token_headers, json={"scene_id": SCENE_ID})
        ...
        assert client.post("/api/ncore/downloads", headers=token_headers, json={"scene_id": "22b769dd-b4fa-4d88-ba4e-e6a230ff0c66"}).status_code == 422
