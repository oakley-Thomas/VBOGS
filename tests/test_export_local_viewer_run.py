...
def test_export_local_viewer_run_copies_renderable_layout_without_prepared_images(tmp_path):
    drive = "drive_sync"
    ...
    assert not (output_dir / "prepared" / "images" / "000000.png").exists()
    ...
    assert manifest["prepared_images"]["materialized"] is False
    assert manifest["prepared_images"]["images_copied"] is False
    ...
    assert f"{drive}/prepared/images/000000.png" not in names
...
def test_export_local_viewer_run_can_include_prepared_images_for_source_loader(tmp_path):
    drive = "drive_sync"
    source_model = tmp_path / "OCTREE-ANYGS" / drive / "run-a"
    checkpoint = source_model / "point_cloud" / "iteration_7"
    checkpoint.mkdir(parents=True)
    ...
    write_yaml(
        source_model / "config.yaml",
        {
            "model_params": {
                "source_path": "/data/COLMAP/drive_sync",
                "model_config": {"kwargs": {"gs_attr": "explicit3D"}},
            },
        },
    )
    ...
    manifest = export_local_viewer_run(
        ...
        write_archive=False,
        overwrite=False,
        include_prepared_images=True,
    )

    image_path = output_dir / "prepared" / "images" / "000000.png"
    assert image_path.is_file()
    assert not image_path.is_symlink()
    assert image_path.read_bytes() == b"raw-png"
    assert manifest["prepared_images"]["materialized"] is True
    assert manifest["prepared_images"]["images_copied"] is True
