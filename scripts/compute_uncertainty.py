...
def compute_uncertainty(posterior: Any, *, u_max: float | None, eps: float) -> dict[str, np.ndarray | float | int]:
    ...
    active_mask = np.arange(max_k)[None, :] < final_k[:, None]
    ...
    observed_u = np.sum(np.where(active_mask, weights * component_entropy, 0.0), axis=1)
    ...
    uncertainty[observed_anchor_ids] = observed_u.astype(np.float32)
