...
| Training images | Defaults to the left perspective camera frames from `image_00/data_rect/`. Use `--training-cameras stereo` to add `image_01/data_rect/` as a second posed RGB training camera. | Decodes the selected NCore camera sensor, default `camera_front_wide_120fov`. |
