...
Stage 1 uses upstream Octree-AnyGS with an adapter-produced static-region
alpha mask. Confirmed moving actors have alpha zero, so their pixels do not
contribute to the RGB supervision or downstream sparse seed. The scene is
trained from posed RGB frames prepared in a COLMAP-style layout. Stereo is not
used as a training signal for this stage; dataset-specific depth or LiDAR
observations are used later to build the 6D point cloud.
...
\section{Stage 2: Build a 6D World Point Cloud}

Stage 2 is a dataset adapter. For KITTI-360, each rectified stereo pair is
matched, unprojected into the left-camera frame, transformed to world
coordinates, and colored from the left image. For NVIDIA PhysicalAI AV NCore,
the same contract can be produced from motion-compensated LiDAR points colored
by camera projection, or from a configured camera-depth pair. In all cases the
adapter concatenates world-frame position and RGB color into one 6D point
cloud:
...
The same static-region mask is applied before unprojection. For KITTI-360 it
rejects masked stereo pixels; for NCore it rejects points in confirmed moving
cuboids and camera colors sampled through masked pixels. Candidates without
enough temporal evidence remain valid deliberately: this is a
confirmed-mover filter, not a ``remove every vehicle'' semantic filter.

\begin{lstlisting}
points_world = []                                # (N, 6) = xyz + rgb
Foreach frame in input_data:
    If point_source == stereo:
        pts_world, rgb = StereoToWorld(frame)
    Else If point_source == lidar:
        pts_world, rgb = LidarToWorldAndProjectColor(frame)
    Else If point_source == camera_depth:
        pts_world, rgb = CameraDepthToWorld(frame)
...
\begin{itemize}
  ...
  \item \textbf{Stereo quality.} Stereo depth quality dominates
        ...
        texture thresholds.
  \item \textbf{Continual variant.} The streaming update path uses the same
        fixed-\(K\) sufficient-statistic update contract as
        \path{fit_gmm_step}. It carries per-anchor posterior state across
        batches when updating the uncertainty head online. The first online
        implementation keeps Octree-AnyGS anchors fixed, keeps each observed
        anchor's selected \(K\) fixed, and updates only the per-anchor VBGS
        posterior and scalar uncertainty used by \texttt{render\_scalar}.
