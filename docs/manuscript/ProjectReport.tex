...
\section{Architecture and Runtime Services}

VBOGS deliberately keeps the PyTorch and JAX sides separate. Octree-AnyGS and
VBGS have incompatible CUDA dependency stacks, so the project uses separate
Docker services:
...
framework inside the runtime stack it supports.
...
\midrule
\endhead
M1: Docker runtime setup &
Complete &
Separate Torch and JAX runtimes are represented by repo-owned Docker services.
Compose smoke-test commands exist for both upstream stacks. \\
...
\begin{itemize}
  ...
  \item \path{configs/pipeline/portainer.yaml}: server deployment profile with
        compose-managed Docker volumes and higher-quality training defaults.
