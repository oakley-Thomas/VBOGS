...
`outputs/v1_0/<drive>/<drive>.zip`. The archive excludes the top-level
`views/` directory even when render diagnostics exist in the run output
directory. It also creates a portable local viewer export at
`outputs/v1_0/<drive>/local_viewer/` and includes it in that same zip unless
...
- `local_viewer/` with a renderable checkpoint, prepared camera metadata,
  uncertainty array, and `VIEWER_COMMANDS.md`
